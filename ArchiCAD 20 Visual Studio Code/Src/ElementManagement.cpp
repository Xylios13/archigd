...
void getWalls(){
	...
	for (GS::Array<API_Guid>::ConstIterator it = elemList.Enumerate(); it != NULL; ++it) {
		...
		if (err == NoError) {
			...
			if (element.wall.oppMat.overridden){
				aux->set_oppmat(searchOverrideMaterialsValue(element.wall.oppMat.attributeIndex));
			}
			else{
				aux->set_oppmat("");
			}
			if (element.wall.refMat.overridden){
				aux->set_refmat(searchOverrideMaterialsValue(element.wall.refMat.attributeIndex));
			}
			else{
				aux->set_refmat("");
			}
			if (element.wall.sidMat.overridden){
				aux->set_sidmat(searchOverrideMaterialsValue(element.wall.sidMat.attributeIndex));
			}
			else{
				aux->set_sidmat("");
			}
