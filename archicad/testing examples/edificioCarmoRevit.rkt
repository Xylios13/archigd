#lang racket
(require "archicad.rkt") ;rosetta/revit)
;(delete-all-shapes)
...
#;(define beam-family (load-beam-family "C:\\ProgramData\\Autodesk\\RVT 2015\\Libraries\\Portugal\\Structural Framing\\Wood\\M_Timber.rfa" '("b" "d")))
...
#;(define column-family (load-column-family "C:\\ProgramData\\Autodesk\\RVT 2015\\Libraries\\Portugal\\Columns\\M_Round Column.rfa" '("Diameter")))
...
(define (building-columns p n m)
  (parameterize ((default-column-family (column-family-element column-family #:width column-width)))
    (for/list ((x (range 0 (+ (- building-length column-width) (/ (- building-length column-width) n)) (/ (- building-length column-width) n))))
      (for/list ((y (range 0 (+ (- building-width column-width) (/ (- building-width column-width) m)) (/ (- building-width column-width) m))))
        (column (+xy p
                     (+ x (/ column-width 2))
                     (+ y (/ column-width 2))))))))
  ...
  (let ((ll (level-list p)))
    (map (lambda (level)
           (parameterize ((current-level level)
             ...
             (slab (list p (+x p building-length) (+xy p building-length building-width) (+y p building-width)))
             (building-columns p m n)))
         ll)
    (roof (list p (+x p building-length) (+xy p building-length building-width) (+y p building-width))
    ...
    (for/list ((r (range 0 length (/ length n))))
      (for/list ((z (range 0 height (/ height m))))
        (facade-element-beam (+xz p r (+ z (/ element-height 2)))
                             element-length
                             element-height
                             (mm (+ 2000 (* 1000 (cos (+ (* z 2pi (/ 1 height)) (* r 4pi (/ 1 length))))))))))))

(define (facade-element-beam p width height dist)
  (let ((e (/ height 2)))
    (parameterize ((default-beam-family (beam-family-element beam-family #:width e #:height e)))
      (let* ((p0 p)
