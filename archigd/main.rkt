...
(define (create-polygon-points points)
  (let* ((sides (length points))
         (edges (polygon-edges sides))
         (polygon (internal-polygon sides)))
    (create-morph (u0) points edges polygon)))

;(send (create-polygon (u0) 6 1 0))
;Creates a polygon
(define (create-polygon p sides radius rotation)
  (let* ((points (polygon-points p sides radius rotation))
         (edges (polygon-edges sides))
         (polygon (internal-polygon sides)))
  (create-morph (u0) points edges polygon)))
