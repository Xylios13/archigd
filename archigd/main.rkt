...
;(polygon-edges (polygon-apply-vector (list (xy -1 -1)(xy 1 -1)(xy 1 1)(xy -1 1)) (z 1)))
(define (polygon-edges points)
  (let* ((n-points (length points))
         (sides (/ n-points 2))
         (base-edges (map-division (lambda (n)
                                     (if (= n (- sides 1))
                                         (xy n 0)
                                         (xy n (+ n 1))))
                                   0 sides sides #f))
         (top-edges (map-division (lambda (n)
                                    (if (= n (- n-points 1))
                                        (xy n sides)
                                        (xy n (+ n 1))))
                                  sides n-points sides #f))
         (side-edges (map (lambda (e1 e2)
                            (xy e1 e2))
                          (range 0 sides)
                          (range sides n-points))))
    (append base-edges top-edges side-edges)))

(define (polygon-mesh edges sides)
  (list))

(define (regular-pyramid-points center sides radius rotation height)
  (append (polygon-points center sides radius rotation)
          (list (xyz (cx center) (cy center) height))))

(define (regular-pyramid-edges points sides)
...
(define (regular-pyramid-polygon edges sides)
  (let ((base-polygon (map-division (lambda (n)
                                      (xy n 0))
                                    0 sides sides #f))
        (sides-polygons (map-division (lambda (n)
                                        (if (= n (- sides 1))
                                            (list (xy n 1)
                                                  (xy sides 0)
                                                  (xy (+ n sides) 1))
                                            (list (xy n 1)
                                                  ...
                                                  (xy (+ n sides) 1))))
                                      0 sides sides #f)))
    (append (list base-polygon) sides-polygons)))

(define (create-regular-pyramid center sides radius rotation height)
  (let* ((points (regular-pyramid-points center sides radius rotation height))
         (edges (regular-pyramid-edges points sides))
         (polygons (regular-pyramid-polygon edges sides)))
    (create-morph center 
                  points
                  edges
                  #;polygons
