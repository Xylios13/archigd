...
(require "ArchiGD.rkt")
(require murphy/protobuf1/main)
(require murphy/protobuf1/syntax)
(require murphy/protobuf1/encoding)
(require srfi/26)
       ...
       (parameterize ((current-level level)
         ...
         (let* ((h (current-level-elevation))
           ...
           (if (>= h (- height wallheight))
               (begin
                 (rotate-element-z (create-roof roof-points #:bottom-level (upper-level) #:material "Roof Aluminium") current-rotation)
                 (grid-typical-floor-intersect p d l-corridor d1 t r-small interior-slab-id #f))
               (grid-typical-floor-intersect p d l-corridor d1 t r-small interior-slab-id #t))
           (create-walls-from-slab slab-id (/ wallheight 2) t wall-material)
           ;(create-walls-from-slab interior-slab-id wallheight t wall-material)
           ;(create-cwalls-from-slab interior-slab-id wallheight)
